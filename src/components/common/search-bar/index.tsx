...
function groupByColumn(items: SourceItemProps[]) {
  return items.reduce((acc, item) => {
    ...
    if (m.column === "Công nghệ") return -1
    if (n.column === "Công nghệ") return 1

    if (m.column === "Chưa phân loại") return 1
    if (n.column === "Chưa phân loại") return -1
    ...
    () =>
      groupByColumn(typeSafeObjectEntries(sources)
        ...
        .map(([k, source]) => ({
          id: k,
          title: source.title,
          column: source.column ? columns[source.column].zh : "Chưa phân loại",
          name: source.name,
  ...
  return (
    ...
    >
      <Command.Input
        ref={inputRef}
        autoFocus
        placeholder="Tìm nguồn tin bạn muốn"
      />
      <div className="md:flex pt-2">
        <OverlayScrollbar defer className="overflow-y-auto md:min-w-275px">
          <Command.List>
            <Command.Empty>Không tìm thấy nguồn tin phù hợp. Bạn có thể tạo issue trên GitHub.</Command.Empty>
            {
